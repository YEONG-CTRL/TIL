...
- __safe sequence__ 를 찾아내는 것이 deadlock avoidance 알고리즘의 핵심이다.
...
- LRU구현을 위한 두가지 방법(소프트웨어)
    1. Counter implementation : 페이지를 참조할때마다, counter를 복사하여서, 나중에 victim선정시 가장 작은 값을 가진 페이지를 선정한다.  
    2. Stack : page number를 스택에 집어넣는다. 다만 스택처럼 LIFO는 아니고, 중간에서 페이지가 빠져나간다.  
    두 방법 모두 비용이 과도하게 들고 현실적이지 않다.

- LRU는 하드웨어의 지원이 필요하다 : reference bit
    - reference bit는 초기에 0으로 설정, 페이지 참조가 일어날때마다 1로 세팅해준다.
    - reference bit가 0 인 것을 replace한다.

- Second chance 알고리즘 
    : FIFO replacement를 실행한다, 이때 만약 reference bit가 0이면 replace 해주고, 값이 1이면 second chance를 주고 다음 FIFO를 실행한다.   


### Frame Allocation
1. eqaul allocation / Proportional allocation

2. Global allocation / Local allocation  
: 모든 프레임에서 replacement frame을 고를 것이냐, allocated frames 중 하나를 쓸것이냐.(Victim 선정의 문제)

### Thrashing : 프로세스가 page in/out을 하느라 너무 바빠서 다른 일을 처리하지 못하는 경우.  
- 프로세스가 충분한 페이지를 가지고 있지 못하면 page fault rates가 매우 커진다(degree of multiprogramming)

    <img width="579" alt="image" src="https://user-images.githubusercontent.com/79896709/174730437-2b51e85d-75be-48e2-9c1e-78bb560a2c53.png">

그러다가 갑자기 CPU utilization이 확 떨어지는 현상을 thrashing이라고 부른다.  
왜? 프로세스 개수가 페이지 프레임만큼 많으면, 계속 page fault가 나서, CPU를 사용하지는 않고, 페이지 교체하는데에만 계속 시간을 쓴다.  
